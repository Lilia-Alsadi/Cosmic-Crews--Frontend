...
const events = [
  { id: 1, month: 'JUN', day: '21', title: 'Summer Solstice Gathering', target: 'Milky Way Core', location: 'Dana Reserve', isRsvpd: true },
  { id: 2, month: 'JUL', day: '14', title: 'Supermoon Watch Party', target: 'Lunar Craters', location: 'Wadi Rum Observatory', isRsvpd: false },
  { id: 3, month: 'AUG', day: '12', title: 'Perseids Meteor Shower', target: 'Perseus Constellation', location: 'Azraq Wetland', isRsvpd: false },
];
      ...
      <div className="flex flex-col gap-4 overflow-y-auto custom-scrollbar pr-2 pb-20">
        {events.map((event) => (
          <div key={event.id} className="flex flex-col p-4 bg-[#0F1428]/80 backdrop-blur-md border border-slate-800 rounded-xl hover:border-slate-700 transition-colors shadow-lg">
            ...
            <button 
              className={`mt-4 w-full py-2 rounded-lg font-bold text-sm transition-all flex items-center justify-center gap-2 ${
                event.isRsvpd 
