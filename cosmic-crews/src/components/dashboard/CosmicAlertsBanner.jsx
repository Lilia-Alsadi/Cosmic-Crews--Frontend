  ...
  return (
    <div className="relative w-full overflow-hidden rounded-xl bg-[#0a0616] p-[1px] mb-6 shadow-lg transition-all duration-500">
      ...
      <div className="relative flex items-start gap-4 rounded-xl bg-[#0B1021] p-4">
        <div className="flex-shrink-0 mt-1">{alertConfig.icon}</div>

        <div className="flex-1">
          <h3 className={`text-lg font-bold ${alertConfig.textColor}`}>{alertConfig.title}</h3>
          <p className="mt-1 text-sm text-gray-300 leading-relaxed">{alertConfig.desc}</p>
        </div>

        <button onClick={() => setIsVisible(false)} className="flex-shrink-0 text-gray-400 hover:text-white transition-colors p-1">
          <X size={20} />
        </button>
      </div>
    </div>
