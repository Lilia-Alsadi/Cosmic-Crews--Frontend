...
const getMoonPhase = (date = new Date()) => {
  ...
  if (phase < 14) return { name: "Waxing Gibbous", illum: Math.round(50 + ((phase - 7) / 7) * 50) };
  if (phase < 15.5) return { name: "Full Moon", illum: 100 };
  if (phase < 22) return { name: "Waning Gibbous", illum: Math.round(100 - ((phase - 15) / 7) * 50) };
  if (phase < 23.5) return { name: "Last Quarter", illum: 50 };
  return { name: "Waning Crescent", illum: Math.round(50 - ((phase - 22) / 7) * 50) };
};
    ...
    async function fetchSkyConditions() {
      ...
      try {
        const response = await fetch(
          "https://api.open-meteo.com/v1/forecast?latitude=31.9522&longitude=35.9284&current=cloud_cover,relative_humidity_2m"
        );
      ...
      <div className="flex flex-col md:flex-row items-center gap-8 relative z-10">
        <div className="flex items-center gap-6">
          <div className="relative flex items-center justify-center w-32 h-32 rounded-full border-4 border-purple-900/50 shadow-[0_0_30px_rgba(168,85,247,0.3)]">
            <svg className="absolute inset-0 w-full h-full -rotate-90">
              <circle
                ...
                className="drop-shadow-[0_0_8px_rgba(168,85,247,0.8)] transition-all duration-1000 ease-out"
              />
              <defs>
          ...
          <div className="flex flex-col">
            <span className={`px-3 py-1 text-xs font-bold uppercase tracking-wider border rounded-full w-fit mb-2 ${index >= 70 ? "text-green-300 bg-green-900/30 border-green-500/30" : "text-amber-300 bg-amber-900/30 border-amber-500/30"
              }`}>
              {index >= 70 ? "Excellent Conditions" : "Marginal Conditions"}
            </span>
            <p className="text-sm text-gray-400 max-w-[180px]">
              {index >= 70
                ? "Atmospheric stability is high. Optimal viewing for deep-sky objects."
                : "Atmospheric interference detected. Expect lower visibility."}
            </p>
          </div>
        </div>
