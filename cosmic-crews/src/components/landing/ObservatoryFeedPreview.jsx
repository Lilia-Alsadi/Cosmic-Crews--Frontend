...
const ObservationCard = ({ id, image, imageAlt, avatar, username, timeAgo, quote, hearts, comments }) => (
  <article id={id} className="flex flex-col rounded-2xl overflow-hidden bg-slate-900/40 border border-white/5 hover:border-purple-500/20 shadow-lg hover:-translate-y-1 transition-all duration-300 group">
    <div className="relative aspect-[16/10] overflow-hidden">
      ...
      <span className="absolute top-3 right-3 text-[11px] text-slate-300 bg-slate-950/60 backdrop-blur-md px-2 py-0.5 rounded-full">{timeAgo}</span>
    </div>
    ...
    <div className="relative flex flex-col px-5 pb-5 pt-8 bg-slate-950/80 border-t border-white/5 flex-grow justify-between">
      <div className="absolute -top-5 left-5 flex items-center gap-2">
        <div className="w-10 h-10 rounded-full flex items-center justify-center text-xs font-bold text-white flex-shrink-0" style={{ background: avatar.bg, boxShadow: "0 0 0 2px #0a0e1a" }} aria-label={`${username}'s avatar`}>
          {avatar.initials}
        </div>
        <span className="text-sm font-semibold text-white">{username}</span>
      </div>

      <p className="italic text-sm text-slate-400 leading-relaxed min-h-[50px] mt-1">&ldquo;{quote}&rdquo;</p>

      <div className="flex items-center gap-4 pt-3 mt-4 border-t border-white/5 text-xs text-slate-500">
        <button id={`${id}-heart`} className="flex items-center gap-1.5 hover:text-pink-400 transition-colors duration-200" aria-label={`${hearts} likes`}>
          <HeartIcon className="w-3.5 h-3.5" />
          {hearts}
        </button>
        <button id={`${id}-comment`} className="flex items-center gap-1.5 hover:text-sky-400 transition-colors duration-200" aria-label={`${comments} comments`}>
  ...
  <section id="observatory-feed" className="relative py-24" aria-label="Observatory Feed">
    <div className="section-container">
      <div className="flex items-end justify-between mb-10">
        <div>
          <h2 className="font-display text-3xl font-bold text-white mb-2">What crews are seeing right now</h2>
      ...
      <div className="grid gap-6 grid-cols-3">
        {CARDS.map((card) => (
          <ObservationCard key={card.id} {...card} />
        ))}
      </div>
    </div>
  </section>
