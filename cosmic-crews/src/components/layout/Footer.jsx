...
const SOCIALS = [
  {
    ...
    Icon: TwitterIcon,
    label: "X / Twitter",
    hoverClass: "hover:text-sky-400 hover:border-sky-400/30",
  },
  {
    id: "footer-github",
    Icon: GithubIcon,
    label: "GitHub",
    hoverClass: "hover:text-slate-100 hover:border-slate-100/30",
  },
  {
    id: "footer-instagram",
    Icon: InstagramIcon,
  ...
  },
  {
    ...
    Icon: DiscordIcon,
    label: "Discord",
    hoverClass: "hover:text-indigo-400 hover:border-indigo-400/30",
  },
];
    ...
    <div className="section-container pt-14 pb-0">
      <div className="grid gap-10 pb-12 grid-cols-1 sm:grid-cols-2 md:grid-cols-4">
        <div className="flex flex-col gap-5">
          ...
          <p className="text-sm text-slate-500 leading-relaxed max-w-[220px]">A quiet corner of the internet for people who love looking up.</p>
          <div className="flex items-center gap-2" aria-label="Social links">
            {SOCIALS.map(({ id, Icon, label, hoverClass }) => (
              <a key={id} id={id} href="#" aria-label={label} className={`w-9 h-9 rounded-xl flex items-center justify-center text-slate-500 border border-white/5 bg-white/[0.02] hover:bg-white/[0.06] transition-all duration-200 ${hoverClass}`}>
      ...
      <div className="flex items-center justify-between py-5 border-t border-white/5">
        <p className="text-xs text-slate-600">© {new Date().getFullYear()} Cosmic Crews. All rights reserved.</p>
      </div>
    </div>
  </footer>
);
