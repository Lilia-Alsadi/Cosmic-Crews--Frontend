...
const SkyMilesWidget = () => {
  const { user } = useAuth();
  const [stats, setStats] = useState({ logs: 0, stars: 0, crews: 0 });
      ...
      <div className="flex flex-col md:flex-row items-center justify-between gap-8 relative z-10">
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col">
            <h2 className="text-xl font-bold text-white tracking-wide">Cosmic Contributions</h2>
            <p className="text-sm font-medium text-purple-400">Platform Statistics</p>
          </div>
        </div>

        <div className="flex items-center gap-8 md:gap-12 bg-slate-900/40 px-8 py-4 rounded-xl border border-slate-800/50">
          <div className="flex flex-col items-center">
            <span className="text-3xl font-black text-white">{stats.logs}</span>
          ...
          <div className="flex flex-col items-center">
            <span className="text-3xl font-black text-white">{stats.crews}</span>
